...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  Future<void> verifyPhoneNumber(String phoneNumber) async {
    print('🔍 Firebase OTP Debug - Sending OTP to: $phoneNumber');
    final FirebaseAuth firebaseAuth = FirebaseAuth.instance;
    await firebaseAuth.verifyPhoneNumber(
      phoneNumber: phoneNumber,
      verificationCompleted: (PhoneAuthCredential credential) async {
        print('🔍 Firebase OTP Debug - Auto verification completed');
        firebaseCredentials = credential;
      },
      verificationFailed: (FirebaseAuthException e) {
        print(
            '🔍 Firebase OTP Debug - Verification failed: ${e.code} - ${e.message}');
      ...
      codeSent: (String verificationId, int? resendToken) async {
        print(
            '🔍 Firebase OTP Debug - OTP sent successfully, verificationId: $verificationId');
      ...
      codeAutoRetrievalTimeout: (String verificationId) {
        print('🔍 Firebase OTP Debug - Auto retrieval timeout');
      },
    );
  }
  ...
  Future<void> _signInWithOTP(
    ...
    print('🔍 OTP Debug - Firebase enabled: ${firebaseEnabled.value}');
    print(
        '🔍 OTP Debug - isFirebaseOtpVerifyEnable: $isFirebaseOtpVerifyEnable');
    print('🔍 OTP Debug - isLoginByEmail: ${event.isLoginByEmail}');

    if (firebaseEnabled.value == true && !event.isLoginByEmail) {
      if (isFirebaseOtpVerifyEnable && !event.isLoginByEmail) {
        print('🔍 OTP Debug - Using Firebase OTP verification');
        ...
        print('🔍 OTP Debug - Using Server-side OTP verification');
      ...
      print('🔍 OTP Debug - Using Demo OTP mode (123456)');
      isLoading = false;
  ...
  Future<void> _confirmOrVerifyOTP(
    ...
    print('🔍 OTP Verify Debug - Firebase enabled: ${firebaseEnabled.value}');
    print(
        '🔍 OTP Verify Debug - isFirebaseOtpVerifyEnable: $isFirebaseOtpVerifyEnable');
    print('🔍 OTP Verify Debug - isLoginByEmail: ${event.isLoginByEmail}');
    print('🔍 OTP Verify Debug - OTP entered: ${event.otp}');

    if (event.otp.isNotEmpty) {
      if (firebaseEnabled.value == true && !event.isLoginByEmail) {
        if (isFirebaseOtpVerifyEnable && !event.isLoginByEmail) {
          print('🔍 OTP Verify Debug - Verifying with Firebase');
          try {
          ...
          print('🔍 OTP Verify Debug - Verifying with Server API');
          if (!event.isLoginByEmail) {
        ...
        if (event.isLoginByEmail) {
          ...
          // DEMO LOGIN
          print('🔍 OTP Verify Debug - Using Demo OTP verification (123456)');
          if (event.isUserExist &&
              event.otp == '123456' &&
