...
Future<void> _initializeFirebase() async {
  try {
    Firebase.app();
    printWrapped("Firebase already initialized");
  } catch (e) {
    printWrapped("Initializing Firebase...");
    ...
    printWrapped("Firebase initialized successfully");
  }
}
...
void onStart(ServiceInstance service) async {
  // Background service started - just collect location data
  printWrapped("Background service started");
  ...
  Timer.periodic(const Duration(minutes: 10), (timer) async {
    try {
      ...
      if (userData != null && userData!.id != null) {
        ...
        printWrapped(
            "Location collected: ${position.latitude}, ${position.longitude}");

        // Store location data locally or send to main app via method channel
        // Firebase updates will be handled by the main app when it's active
      } else {
        printWrapped("User data not available, skipping location collection");
      }
    } catch (e) {
      printWrapped("Error collecting location: $e");
    }
  });
}
